...
const Game = () => {
  ...
  const checkWinConditionAchieved = (task) => {
    if (
      Object.keys(task.requirements).reduce((acc, key) => {
          ...
          <div className="status-bar-services-wrapper">
            <div className="status-bar-services">
              {[...Array(6).keys()].map((key) => (
                <div key={key} className="status-bar-services-item">
                  {task.services?.[key] ? <Service service={task.services[key]}>{task.services[key].name}</Service> : <div className="plus plus-wrapper"></div>}
                </div>
              ))}
            </div>
